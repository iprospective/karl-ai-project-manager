...
| iprospective/infra | 🟠 à faire | **norme `<projet>/dev`** (comme Worm) : `infra` → `iprospective/infra/` (projet) + `dev/` (code) ; `.mmi-pm` au niveau projet ; repo `infra-core` (PM `infrastructure`) ; root + co-loc + **18 sessions** (`-zfs-workspaces-infra` → `-zfs-workspaces-iprospective-infra-dev`) |
