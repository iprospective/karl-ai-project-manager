...
# Normes de gestion des tâches — v1.22.0
...
**Règle** : à **chaque commit** rattaché à une tâche PM, reporter sur le ticket
Redmine le **delta** consommé depuis le commit précédent, sous forme d'une
**saisie de temps** (`POST /time_entries.json`) :
...
**Note Redmine systématique par commit (obligatoire).** Le `.log.md` n'est pas
lu par les humains et la time_entry est peu lisible. Donc **tout commit rattaché
à une tâche s'accompagne d'une note Redmine** sur le ticket, concise et
human-readable, portant :

- la **référence du commit** : SHA court + message, et l'**URL GitLab** complète
  du commit (cliquable) quand le repo est sur GitLab ;
- le **temps passé** et les **tokens** de l'incrément (le delta depuis le commit
  précédent — les mêmes valeurs que la time_entry) ;
- une ligne de **quoi a été livré/modifié** par ce commit.

> Articulation avec la règle anti-bruit (§ « Double traçabilité ») : la note de
> commit est un cas **systématique et distinct** des notes de synthèse
> discrétionnaires (décision/arbitrage/jalon) — elle porte une livraison
> concrète et ses métriques, donc a toujours « quelque chose à dire ». Pour ne
> pas spammer, plusieurs **petits commits WIP rapprochés** d'une même session
> PEUVENT être regroupés en **une seule** note listant chaque commit (ref +
> delta cumulé) ; un commit de **livraison/jalon** a toujours sa propre note.
...
- **Exception systématique — le commit** : la note discrétionnaire décrite ici
  concerne les *échanges/jalons*. Le **commit**, lui, déclenche **toujours** une
  note (ref commit + temps + tokens) — cf. § « ROI assisté par IA » →
  « Journalisation par commit ». Ce n'est pas du bruit : c'est une livraison tracée.
